...
const ModernStory = ({ onBack, onBackToTimeline }) => {
...
useEffect(() => {
  ...
  if (isAuthenticated) {
    const newStats = {
      ...
      statName1: Math.max(0, Math.min(100, stats.duty + (statChanges.duty || 0))),
      statName2: Math.max(0, Math.min(100, stats.survival + (statChanges.survival || 0))),
      statName3: Math.max(0, Math.min(100, stats.truth + (statChanges.truth || 0)))
    };
